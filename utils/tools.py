...
class WikiDataset(Dataset):
    def __init__(self, sentences_path, max_samples=None):   # 对WikiDataset进行动态下采样，max_samples=wiki的batch_size * ts数据集的迭代次数
    ...
    def __len__(self):
        ...
        return min(total_len, self.max_samples) if self.max_samples is not None else total_len
...
def text_data_provider(args, ts_iter_count=None):
    ...
    text_dataset = WikiDataset(sentences_path=args.sentences_path, max_samples= max_samples)
