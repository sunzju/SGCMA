...
def parse_args():
    ...
    parser.add_argument('--cluster_num', type=int, default=100, help='cluster number')
...
if __name__ == '__main__':
    args = parse_args()
    model = SGCMA.Model(args).cuda()
    model.load_state_dict(torch.load('checkpoints/hmm_100_checkpoint.pth', weights_only=False), strict=False)
    torch.save(model.wiki_hmm.state_dict(), 'checkpoints/hmm_100_checkpoint.pth')
    print(model)
